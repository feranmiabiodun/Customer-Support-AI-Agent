...
def get_agent(headless: Optional[bool] = None, slow_mo: Optional[int] = None, user_data_dir: Optional[str] = None) -> CoreAgent:
    return CoreAgent(headless=headless, slow_mo=slow_mo, user_data_dir=user_data_dir)
...
def create_ticket(intent: Dict[str, Any], provider: str, dry_run: bool = False, **kwargs) -> Dict[str, Any]:
    ...
    Args:
      ...
      kwargs: forwarded to CoreAgent (headless, slow_mo, user_data_dir, timeout)

    Returns:
      dict: canonical result from CoreAgent.create_ticket or adapter fallback result
    """
    if not provider:
